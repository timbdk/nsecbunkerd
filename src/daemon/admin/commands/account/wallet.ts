...
export async function generateWallet(walletConfig: IWalletConfig, username: string, domain: string, npub: string) {
...
export async function generateLNBitsWallet(lnbitsConfig: LNBitsWalletConfig, username: string, domain: string, npub: string) {
  ...
  return await generateLNAddress(username, domain, wallet.inkey, npub, 'lnbits', lnbitsConfig.url, lnbitsConfig.nostdressUrl)
}
