...
export async function processRequestWebHandler(request, reply) {
  ...
  await allowAllRequestsFromKey(record.remotePubkey, record.keyName, record.method, undefined, undefined, allowScope)

  if (record.method === 'connect') {
    debug('connect, adding sign_event capability')
    await allowAllRequestsFromKey(record.remotePubkey, record.keyName, 'sign_event', undefined, undefined, allowScope)
  }
...
export async function processRegistrationWebHandler(request, reply) {
  try {
    ...
    await createUserRecord(body.username, body.domain, createdPubkey, body.email, body.password)
    ...
    await allowAllRequestsFromKey(record.remotePubkey, record.keyName, record.method, undefined, undefined)

    // redirect to callbackUrl
    if (callbackUrl) {
      return reply.view('/templates/redirect.handlebar', { callbackUrl }).redirect(callbackUrl)
    }
...
async function createUserRecord(username: string, domain: string, pubkey: string, email: string, password: string) {
